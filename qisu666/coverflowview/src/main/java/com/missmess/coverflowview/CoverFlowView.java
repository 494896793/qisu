...
//        Log.v("CoverFlowView", "onLayout");
        ...
        if (mDataChanged) {
            ...
            if (lastMidIndex + 1 == mid) { //右滑至item出现了
                ...
                try{midView.bringToFront();}catch (Throwable tt){ tt.printStackTrace(); }
    ...
    private View layoutLeftChild(int position, float offset) {
        ...
        if (child != null) {
            makeChildTransformer(child, actuallyPosition, offset, true);
        }
        return child;
    }
    ...
    private View layoutRightChild(int position, float offset) {
        ...
        View child = showViewArray.get(actuallyPosition);
        if (child != null) {
            makeChildTransformer(child, actuallyPosition, offset, false);
        }

        return child;
    }

    private void makeChildTransformer(View child, int position, float offset ,boolean isFromLeft) {
        ...
        float testRotate = 0;
        if (!isFromLeft){
            testRotate = - Math.abs(offset) * 0.25f * 60;
        }else
        {
        ...
        ViewHelper.setRotationY(child, testRotate);
        //        child.setPivotX(0);
