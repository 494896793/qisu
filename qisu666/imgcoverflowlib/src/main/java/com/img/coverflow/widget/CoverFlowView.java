...
public class CoverFlowView extends RelativeLayout {
    ...
    private void triggerOnPageChange() {
        if (mOnPageChangeListener != null) {
            mOnPageChangeListener.onPageSelected(getTopViewPosition());
        }
    }
    ...
    private View layoutLeftChild(int position, float offset) {
        ...
        if (child != null) {
            makeChildTransfromer(child, actuallyPosition, offset,true);
        }
        return child;
    }
    ...
    private View layoutRightChild(int position, float offset) {
        ...
        View child = showViewArray.get(actuallyPosition);
        if (child != null) {
            makeChildTransfromer(child, actuallyPosition, offset,false);
        }
     ...
     * @param offset
     */
    private void makeChildTransfromer(View child, int position, float offset ,boolean isFromLeft) {
        ...
        float testRotate = 0;
        if (!isFromLeft){
            testRotate = - Math.abs(offset) * 0.25f * 90;
        }else
            {
            testRotate =  Math.abs(offset) * 0.25f * 90;
        }
        ...
        //放大缩小动画
//        Log.w("guanglog", "position " + position);
//        Log.w("guanglog", "childHeightScale " + childHeightScale);
//        Log.w("guanglog", "offset " + offset);
//        Log.w("guanglog", "testRotate " + testRotate);
        ...
        ObjectAnimator anim3 = ObjectAnimator.ofFloat(child, "rotationY",0, testRotate);
...
//        child.setPivotY(child.getHeight() / 2);
        ...
        if (isFromLeft){
            child.setTranslationX(translateX  - dp2px(child.getContext(), 60));
        }else {
    ...
    private int dp2px(Context context, float dpSize) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (touchViewItem != null){
                    touchViewItem.getHitRect(topRect);
                }
...
//                }
    ...
    private boolean inRangeOfTopRect(MotionEvent ev) {
        if (topRect.contains((int) ev.getX(), (int) ev.getY())) {
            return true;
        }
        return false;
    }
    ...
    private void touchEnded(MotionEvent event) {
        ...
        if (mTouchMoved || (mOffset - Math.floor(mOffset)) != 0) {
            ...
            onTouchMove = false;
            if (inRangeOfTopRect(event)){
                if (mTopViewClickListener != null){
                    mTopViewClickListener.onClick(getTopViewPosition(), null);
                }
            }
        }
