 ...
 */
public class MoneyListViewActiviAdapter extends BaseAdapter {

    private List<ActiviMoneyBean> list;
    private Context mContext;

    public MoneyListViewActiviAdapter(List<ActiviMoneyBean> list, Context mContext){
        this.list=list;
        this.mContext=mContext;
    }

    public void refreshData(List<ActiviMoneyBean> list){
                  this.list=list;
                  notifyDataSetChanged();
    }

    public void loadData(List<ActiviMoneyBean> list){
        this.list.addAll(list);
        notifyDataSetChanged();
    }

    public List<ActiviMoneyBean> getData(){
               return list;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        holder.tx_money.setText(list.get(position).getConsumeAmount());
        if (list.get(position).getOperationDate()!=null&&!list.get(position).getOperationDate().trim().equals("")) {
            holder.tx_date.setText(list.get(position).getOperationDate().substring(0,10));
        }
        holder.tx_now_money.setText(list.get(position).getMonthAbleaMount());
        holder.tx_pay.setText(list.get(position).getActiveTitle());
        return convertView;
    }
